  ...
  void hlogger::initialize() {
...

    {
      ...
      if (config_->get_type_mask() & htypes_mask::file_daily) {
        ...
        if (config_->get_sinks_info().synchronize) {
          sink_file_daily = std::make_shared<spdlog::sinks::daily_file_sink_mt>(log_filename,
                                                                                info.fs.rotation_hour,
                                                                                info.fs.rotation_minute,
                                                                                info.fs.truncate,
                                                                                info.fs.max_files_by_date);
        } else {
          sink_file_daily = std::make_shared<spdlog::sinks::daily_file_sink_st>(log_filename,
                                                                                info.fs.rotation_hour,
                                                                                info.fs.rotation_minute,
                                                                                info.fs.truncate,
                                                                                info.fs.max_files_by_date);
