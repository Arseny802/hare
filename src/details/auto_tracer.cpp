...
namespace hare::details {
  ...
  inline void auto_measure::on_deinitialized() const noexcept {
    ...
    if constexpr (enable_measure_pretty_format) {
      const auto elapsed_nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(end - started_).count();
      if (elapsed_nanoseconds / 1000000 > 1) {
        const auto elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(end - started_).count();
        if (elapsed_milliseconds / 10000 > 1) {
          const auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(end - started_).count();
          elapsed_output = fmt::format("{} seconds ({} milliseconds)", elapsed_seconds, elapsed_milliseconds);
        } else {
          const auto elapsed_microseconds =
              std::chrono::duration_cast<std::chrono::microseconds>(end - started_).count();
          elapsed_output = fmt::format("{} milliseconds ({} microseconds)", elapsed_milliseconds, elapsed_microseconds);
        }
      } else {
      ...
      elapsed_output =
          fmt::format("{} ms", std::chrono::duration_cast<std::chrono::milliseconds>(end - started_).count());
    }

    // const auto elapsed_microseconds = std::chrono::duration_cast<std::chrono::microseconds>(end - started_);
    // const auto elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(end - started_);
    // const auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(end - started_);
    if (std::uncaught_exceptions() > 0) {
      logger_->trace("Function {} mesure finished{} with {} uncaught exceptions in file {}. Elapsed {}.",
                     function_signature_.c_str(),
                     (obj_name_.empty() ? "" : " for " + obj_name_),
                     std::uncaught_exceptions(),
                     file_name_,
