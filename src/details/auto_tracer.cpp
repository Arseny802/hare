...
namespace hare::details {
  ...
  auto_tracer::auto_tracer(const std::string& logger_name,
  ...
  auto_tracer::~auto_tracer() {
    if (std::uncaught_exceptions() > 0) {
      ...
      logger_->trace("Function {} left in file {}.", function_signature_, file_name_);
    }
  }
  ...
  inline void auto_tracer::on_initialized() noexcept {
    if (std::uncaught_exceptions() > 0) {
      ...
      logger_->trace("Function {} entered in file {}.", function_signature_, file_name_);
    }
  }
  ...
  auto_measure::auto_measure(const std::string& logger_name,
                             std::string_view file_name,
                             std::string_view function_signature):
      logger_(logger_fabric::get_logger(logger_name)),
      file_name_(get_local_file_name(file_name)),
  ...
  inline void auto_measure::on_initialized() noexcept {
    if (std::uncaught_exceptions() > 0) {
      ...
      logger_->trace("Function {} mesure startered in file {}.", function_signature_, file_name_);
    }
