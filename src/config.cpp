...
config::config(std::string project_name, htypes_mask_t htypes_mask, hare::hlevels level)
    : project_name_(std::move(project_name)), logger_min_level_(level), htypes_mask_(htypes_mask) {
}
...
std::string config::create_logger_name(const std::string& project_name,
                                       const std::string& module_name) {
  std::string name =
      project_name.empty() ? config_default::get_default_project_name() : project_name;
...
bool config::parse_logger_name(const std::string& logger_name, std::string& project_name,
